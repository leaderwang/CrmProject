...
namespace Lib
{
    ...
    public class PermissionHelper
    {
        ...
        public bool Write()
        {
            ...
            if (site.IsFromFile == true)
            {
                ...
                try
                {
                    ...
                    {
                        var mlts = new MenuLogic().GetMenus(new Menu() { IsDeleted = false });
                        if (mlts != null)
                        {
                            _Menus = mlts;
                    ...
                    {
                        var pmlts = new PermissionMapLogic().GetPermissionMaps(new PermissionMap() { IsDeleted = false });
                        if (pmlts != null)
                        {
                    ...
                    {
                        var pdlts = new PermissionDataLogic().GetPermissionDatas(new PermissionData() { IsDeleted = false });
                        if (pdlts != null)
                        {
        ...
        private void Read()
        {
            ...
            Thread mt = new Thread(new ThreadStart(() =>
            {
                ...
                if (!hasFile)
                {
                    try
                    {
                        _Menus = new MenuLogic().GetMenus(new Menu() { IsDeleted = false });
                    }
                    catch { }
                }
            }));
            Thread pmt = new Thread(new ThreadStart(() =>
            {
                ...
                if (!hasFile)
                {
                    try
                    {
                        _PermissionMaps = new PermissionMapLogic().GetPermissionMaps(new PermissionMap() { IsDeleted = false });
                    }
                    catch { }
                }
            }));
            Thread pdt = new Thread(new ThreadStart(() =>
            {
                ...
                if (!hasFile)
                {
                    try
                    {
                        _PermissionDatas = new PermissionDataLogic().GetPermissionDatas(new PermissionData() { IsDeleted = false });
                    }
                    catch { }
                }
            }));
        ...
        private static void AutoGenerateMenuAndMap(string title, string url)
        {
            ...
            try
            {
                menuID = db.QueryForObjectSql<Menu>("SELECT * FROM [dbo].[Menu] AS m WHERE [Url] LIKE '" + url.ToLower() + "'").ID;
            ...
            try
            {
                pmt = db.QueryForObjectSql<PermissionMap>("SELECT * FROM [dbo].[PermissionMap] AS pm WHERE [MID]=" + menuID + " AND [Name]='" + title + "'");
