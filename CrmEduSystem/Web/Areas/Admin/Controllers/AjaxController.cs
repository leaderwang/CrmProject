...
namespace Web.Areas.Admin.Controllers
{
    public class AjaxController : Controller
    {
        ...
        public bool validatorExists(string table, string row, string value, int? id)
        {
            ...
            return Convert.ToInt32(new DbUtility.DBContext().ExecuteScalarSql(sql)) == 0 ? false : true;
        }
        /// <summary>
        /// 自动完成
        /// </summary>
        ...
        public ActionResult autoComplete(string words, string table, string field)
        {
            ...
            try
            {
                ...
                var dt = new DbUtility.DBContext().ExecuteDataTableSql(sql);
        ...
        public ActionResult HasPermission(string titleStr, string urlStr)
        {
            ...
            {
                if (Lib.PermissionHelper.IsPast(titleArray[i], urlArray[i]) == true)
                {
                    result += "1,";
                }
                else
        ...
        public ActionResult GetProvinces()
        {
            ...
            Models.AreaLogic al = new Models.AreaLogic();
            var alts = al.GetAreas(new Models.Area()
            {
                Depth = 0,
                IsDeleted = false
            });
        ...
        {
            string result = "1";
            var member = new MemberLogic().GetMember(new Member() { UserName = username, IsDeleted = false });
