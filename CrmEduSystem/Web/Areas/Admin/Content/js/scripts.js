...
function pagination_init() {
    ...
    $(".dt_footer .pagination a").click(function () {
        ...
        if (href != undefined && href != "") {
            ...
            $.get(href, { "rndDate": (new Date()).getMilliseconds() }, function (data) {
                div.html(data);
                sort_init();
                pagination_init();
                box.close();
...
function dt_header_btn_group_init() {
    //导出
    $(".dt_header .icol-doc-excel-csv").parents("a.btn").click(function () {
    ...
    /*搜索*/
    $(".dt_header form [type='submit']").click(function () {
        ...
        $.post(url, param, function (data) {
            if (fun) {
                ...
                if (arr.length > 3 && arr[3] != "") {
                    art.dialog({ title: "温馨提醒", lock: false, icon: icon, dbClose: false, cancel: true, content: arr[1] }).time(showtime);
                }
